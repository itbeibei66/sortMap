...
func (s *SortMap) GetRangeKeyV2(begin, end int64) []int64 {
	ans := make([]int64, 0)
	left, ok := s.SearchRightKey(begin)
	if !ok {
		return nil
	}
	for left <= end {
		ans = append(ans, left)
		left, ok = s.SearchRightKey(left + 1)
		if !ok {
			return ans
		}
	}
...
func (s *SortMap) GetRangeKey(begin, end int64) []int64 {
	...
	dfsCon := &dfsCon{}
	validMap := make(map[int64]struct{})
	s.dfs(sNode, sNode, eNode, (int64(len(sNode.son))-1)*(1-divAbs(sNode.key))/2, dfsCon, validMap)
	return dfsCon.data
}
...
func (s *SortMap) dfs(cur, startNode, endNode *Node, index int64, con *dfsCon, validMap map[int64]struct{}) {
	...
	if cur == nil {
		if index < 19 {
			s.dfs(s.head[index+1], startNode, endNode, int64(len(s.head)-1)*(1-(index+1)/10), con, validMap)
		}
		return
	}
	// 先把自己和子孙节点遍历
	if cur != s.head[index] && cur.key >= startNode.key && cur.key <= endNode.key {
		if _, ok := validMap[cur.key]; !ok {
			con.data = append(con.data, cur.key)
			validMap[cur.key] = struct{}{}
		}
	}
	if cur.son != nil {
		i := index
		for i >= 0 && i < int64(len(cur.son)) {
			if cur.son[i] != nil {
				s.dfs(cur.son[i], startNode, endNode, (int64(len(cur.son[i].son))-1)*(1-divAbs(cur.son[i].key))/2, con, validMap)
			}
			if cur.par == nil {
	...
	// 再遍历往上的邻居节点
	if cur.par == nil {
		s.dfs(cur.par, startNode, endNode, int64(cur.index), con, validMap)
	} else {
		s.dfs(cur.par, startNode, endNode, int64(cur.index)+divAbs(cur.key), con, validMap)
	}
}
